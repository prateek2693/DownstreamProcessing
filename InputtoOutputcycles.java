...
public class InputtoOutputcycles extends JFrame implements ActionListener
{
	JButton submit;
	JButton clear;	
	JPanel panel1;
	...
	public InputtoOutputcycles()
	{
		this.setTitle("Input to Output with Number of Cycles rounded to the nearest Integer");
	...
	public void actionPerformed(ActionEvent ae) 
	{
		...
		*/
		...
		double time1 = 168;
		int no_of_cycles_c_r;
		int no_of_cycles_i_r;
		int no_of_cycles_p_r;
		int batches_per_year_r;
		double product_4_f;
		double product_1_f;
		double product_3_f;
		...
		int no_of_cycles_c_max = 0;
		int no_of_cycles_i_max = 0;
		int no_of_cycles_p_max = 0;
		int batches_per_year_max = 0;
		...
		for(i=harvest_l ; i<=harvest_u ; i=i+harvest_inc)
		{
			...
			for(j=capture_binding_capacity_l ; j<=capture_binding_capacity_u ; j = j+capture_binding_capacity_inc) 
			{
				...
				if (Math.floor(no_of_cycles_c) == Math.floor(no_of_cycles_c + 0.5))
				{
					no_of_cycles_c_r = (int)Math.floor(no_of_cycles_c);
					product_1_f = load_amount_c/no_of_cycles_c_r;  
				}
				else
				{
					no_of_cycles_c_r = (int)Math.floor(no_of_cycles_c + 0.5);
					product_1_f = product_1;
				}				
				
				//Test the rounding off of no.of cycles
				System.out.println("Before: " + no_of_cycles_c);
				System.out.println("After: " + no_of_cycles_c_r); 
				...
				for(k = capture_flow_rate_l ; k<=capture_flow_rate_u ;k=k+capture_flow_rate_inc) 
				{
					...
					double total_time_step_c = (equilib_4cv_c + loading_c + washing_c + elution_3cv_c + regeneration_2cv_c)*no_of_cycles_c_r;
					...
					{ 
						...
						product_2 = step_yield_2 * product_1_f;
						...
						for(m=intermediate_binding_capacity_l ; m<=intermediate_binding_capacity_u ; m=m+intermediate_binding_capacity_inc) 
						{
							...
							if (Math.floor(no_of_cycles_i) == Math.floor(no_of_cycles_i + 0.5))
							{
								no_of_cycles_i_r = (int)Math.floor(no_of_cycles_i);
								product_3_f = load_amount_i/no_of_cycles_i_r;  
							}
							else
							{
								no_of_cycles_i_r = (int)Math.floor(no_of_cycles_i + 0.5);
								product_3_f = product_3;
							}
							...
							for(n=intermediate_flow_rate_l ; n<=intermediate_flow_rate_u ; n=n+intermediate_flow_rate_inc)
							{
								...
								double total_time_step_i = (equilib_4cv_i+loading_i+washing_i+elution_10cv_i+regeneration_2cv_i) * no_of_cycles_i_r;
								...
								for(o=intermediate_ph_l ; o<=intermediate_ph_u ; o = o + intermediate_ph_inc)
								{
									...
									double product_4=step_yield_4*product_3_f;
									...
									for(p = polishing_binding_capacity_l ; p<=polishing_binding_capacity_u ; p = p + polishing_binding_capacity_inc)
									{
										...
										if (Math.floor(no_of_cycles_p) == Math.floor(no_of_cycles_p + 0.5))
										{
											no_of_cycles_p_r = (int)Math.floor(no_of_cycles_p);
											product_4_f = load_amount_p/no_of_cycles_p_r;  
										}
										else
										{
											no_of_cycles_p_r = (int)Math.floor(no_of_cycles_p + 0.5);
											product_4_f = product_4;
										}
										...
										for(q=polishing_flow_rate_l ; q<=polishing_flow_rate_u ; q=q+polishing_flow_rate_inc)
										{
											...
											for(r=polishing_ph_l ; r<=polishing_ph_u ; r = r + polishing_ph_inc)
											{
												...
												double product_5 = step_yield_5 * product_4_f;
												
												//test
												//System.out.println("product_5: " + product_5);
												...
												if (Math.floor(batches_per_year) == Math.floor(batches_per_year))
												{
													batches_per_year_r = (int)Math.floor(batches_per_year);
													total_product = product_5 * batches_per_year_r;
												}
												else
												{
													batches_per_year_r = (int)Math.floor(batches_per_year);
												...
												//test
												//System.out.println("total_product: " + total_product);
												
												revenue = sale_price * total_product; 
												
												no_of_cycles_y_c = no_of_cycles_c_r * batches_per_year_r; 
												no_of_cycles_y_i = no_of_cycles_i_r * batches_per_year_r; 
												no_of_cycles_y_p = no_of_cycles_p_r * batches_per_year_r;
													...
													if(no_of_cycles_y_i >= noc_intermediate_l && no_of_cycles_y_i<=noc_intermediate_u)
													{
														if(no_of_cycles_y_p >= noc_polishing_l && no_of_cycles_y_p<=noc_polishing_u)
														{
															if (revenue>revenue_max)
															{
																...
																c_f_r_max=k;
																c_ph_max=l; 
																no_of_cycles_c_max = no_of_cycles_c_r;
																i_b_c_max=m; 
																i_f_r_max=n;
																i_ph_max=o; 
																no_of_cycles_i_max = no_of_cycles_i_r;
																p_b_c_max=p; 
																p_f_r_max=q;
																p_ph_max=r;
																no_of_cycles_p_max = no_of_cycles_p_r;
																batches_per_year_max = batches_per_year_r;
																revenue_max=revenue;
